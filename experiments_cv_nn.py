...
from load_dataset_a import LoadDataset

test = LoadDataset(which='cv')
X = test.data.data
...
for x in range(0, test.shape[0]):
    ...
    np.savetxt("results/nn_dlime_jdist_cv.csv", sim, delimiter=",")
    ...
    np.savetxt("results/nn_lime_jdist_cv.csv", sim, delimiter=",")
    print(np.asarray(sim).mean())
