...
m_index = len(names) - 1
clustered_data = np.column_stack([X, kmeans.labels_])
...
local_preds_avg = np.zeros((test.shape[0],2), dtype=np.float32)
...
for x in range(0, test.shape[0]): #test.shape[0]
    ...
    local_preds_iter = np.zeros((10,2), dtype=np.float32) # , dtype=np.float16
