...
for x in range(0, test.shape[0]):
    ...
    for i in range(0, 10):
        ...
        N = clustered_data[clustered_data[:,19] == clabel[p_label]]
        subset = np.delete(N, 19, axis=1)
    ...
    np.savetxt("results/rf_dlime_jdist_hp.csv", sim, delimiter=",")
    ...
    np.savetxt("results/rf_lime_jdist_hp.csv", sim, delimiter=",")
    print(np.asarray(sim).mean())
